...
from pdf_word_reconstructor.lines_only_region_contract import build_lines_only_region_contract
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Build LINES_ONLY_L2.1 native Word using Mathpix Lines region topology evidence, "
            "with automatic narrow-column floating frames disabled."
        )
    )
...
def main() -> int:
    ...
    artifacts = build_lines_only_region_contract(lines, page_width_pt=float(args.page_width_pt))
    page_structure = artifacts["pageStructure"]
    page_layout_spine = artifacts["pageLayoutSpine"]
    build_contract = artifacts["buildContract"]

    for name, payload in (
        ("LINES_ONLY_L21_PAGE_STRUCTURE.json", page_structure),
        ("LINES_ONLY_L21_PAGE_LAYOUT_SPINE.json", page_layout_spine),
        ("LINES_ONLY_L21_BUILD_CONTRACT.json", build_contract),
    ):
        (output / name).write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")

    docx = output / "lines_only_L21.docx"
    ...
    print("LINES COLUMN DATA   : EVIDENCE ONLY")
    print("NARROW=>FLOAT FRAME : OFF")
    print("REGION TOPOLOGY     : DIAGNOSTIC")
    ...
    print(f"FORMER POSITIONED   : {s.get('formerPositionedUnitCount')}")
    print(f"REGIONS             : {s.get('regionCount')}")
    print(f"MULTICOL CANDIDATES : {s.get('multiColumnCandidateCount')}")
