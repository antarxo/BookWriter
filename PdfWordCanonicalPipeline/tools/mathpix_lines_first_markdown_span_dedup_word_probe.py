...
from pdf_word_reconstructor.lines_first_markdown_span_dedup_contract import build_lines_first_markdown_span_dedup_contract
from pdf_word_reconstructor.source_neutral_builder_adapter import build_source_neutral_document


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Lines-first + Markdown span merge + residual dedup Word probe")
...
def main() -> int:
    ...
    artifacts = build_lines_first_markdown_span_dedup_contract(lines, mmd, page_width_pt=args.page_width_pt)
    write_json(output / "LINES_FIRST_MMD_SPAN_DEDUP_ARTIFACTS.json", artifacts)
    write_json(output / "LINES_FIRST_MMD_SPAN_DEDUP_PAGE_STRUCTURE.json", artifacts["pageStructure"])
    write_json(output / "LINES_FIRST_MMD_SPAN_DEDUP_PAGE_LAYOUT_SPINE.json", artifacts["pageLayoutSpine"])
    write_json(output / "LINES_FIRST_MMD_SPAN_DEDUP_BUILD_CONTRACT.json", artifacts["buildContract"])

    docx_path = output / "lines_first_markdown_span_dedup.docx"
    ...
    write_json(output / "LINES_FIRST_MMD_SPAN_DEDUP_WORD_REPORT.json", report)

    summary = artifacts.get("summary") or {}
    adapter = report.get("sourceNeutralAdapter") or {}
    print("MODE                         : LINES_FIRST_MARKDOWN_SPAN_DEDUP_WORD")
    ...
    print("RESIDUAL DEDUP                : CONSERVATIVE")
