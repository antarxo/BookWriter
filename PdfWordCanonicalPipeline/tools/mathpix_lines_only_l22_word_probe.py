...
from pdf_word_reconstructor.lines_only_region_order_contract import build_lines_only_region_order_contract
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Build LINES_ONLY_L2.2 native Word using Mathpix Lines region topology to "
            "control reading order, with floating frames still disabled."
        )
    )
...
def main() -> int:
    ...
    artifacts = build_lines_only_region_order_contract(lines, page_width_pt=float(args.page_width_pt))
    page_structure = artifacts["pageStructure"]
    page_layout_spine = artifacts["pageLayoutSpine"]
    build_contract = artifacts["buildContract"]

    for name, payload in (
        ("LINES_ONLY_L22_PAGE_STRUCTURE.json", page_structure),
        ("LINES_ONLY_L22_PAGE_LAYOUT_SPINE.json", page_layout_spine),
        ("LINES_ONLY_L22_BUILD_CONTRACT.json", build_contract),
    ):
        (output / name).write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")

    docx = output / "lines_only_L22.docx"
    ...
    print("LINES COLUMN DATA   : TOPOLOGY + ORDER")
    print("NARROW=>FLOAT FRAME : OFF")
    print("REGION ORDER        : ON")
    print("NATIVE REGION RENDER: DEFERRED")
    ...
    print(f"REGION BUNDLES      : {s.get('regionBundleCount')}")
