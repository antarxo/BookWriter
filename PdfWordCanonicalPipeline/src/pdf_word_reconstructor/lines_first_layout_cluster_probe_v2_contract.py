...
VERSION = "lines-first-layout-cluster-probe-0.2"
_EPS = 0.5
...
def _page_top_compositions(items: list[dict[str, Any]], active: list[float]) -> list[dict[str, Any]]:
    ...
    if env is None:
        return []
    # A true top composition must itself remain shallow; a body block merely starting
    # near the top must not drag the composition hundreds of points downward.
    if _h(env) / ah > 0.18:
        return []
...
def _sidebar_candidates(items: list[dict[str, Any]], active: list[float], top_ids: set[str]) -> list[dict[str, Any]]:
    ...
    for side in items:
        ...
        best = None
        for main in mains:
            if main["id"] == side["id"] or _ox(side["box"], main["box"]) > _EPS:
                continue
            overlap = _oy(side["box"], main["box"])
            if overlap <= _EPS:
                continue
            side_persistence = overlap / _h(side["box"])
            main_persistence = overlap / _h(main["box"])
            # Sidebar may begin lower or end earlier than the main lane. What matters is
            # that most of the side material actually runs beside the main material.
            if side_persistence < 0.60:
                continue
            gap = max(0.0, max(side["box"][0], main["box"][0]) - min(side["box"][2], main["box"][2]))
            score = 1.5 * side_persistence + min(0.6, side_h_ratio) + 0.20 * main_persistence
        ...
        confidence = "high" if side_h_ratio >= 0.24 and side_persistence >= 0.75 else "medium"
        out.append({
            ...
            "evidence": ["narrow-prose-lane", "x-disjoint-from-main", "side-mostly-runs-beside-main", "main-lane-wider"],
        })
    return out
...
def _true_column_candidates(items: list[dict[str, Any]], active: list[float], excluded_ids: set[str]) -> list[dict[str, Any]]:
    ...
    for i, a in enumerate(lanes):
        for b in lanes[i + 1:]:
            ...
            if persistence < 0.65:
                continue
            a_substantial = a["itemCount"] >= 2 and a["heightRatio"] >= 0.20
            b_substantial = b["itemCount"] >= 2 and b["heightRatio"] >= 0.20
...
def build_lines_first_layout_cluster_probe_v2_contract(lines_path: Path, mmd_path: Path, page_width_pt: float = 595.276) -> dict[str, Any]:
...
__all__ = ["build_lines_first_layout_cluster_probe_v2_contract"]
