...
VERSION = "lines-first-markdown-span-dedup-contract-0.1"
_MAX_LOOKAHEAD = 3
...
def _is_residual_duplicate(candidate: dict[str, Any], matched_row: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    if len(_norm(fragment)) < _MIN_FRAGMENT_CHARS:
    ...
    accepted = (direct or subsequence) and containment >= _MIN_CONTAINMENT
    return accepted, {
...
def build_lines_first_markdown_span_dedup_contract(
    lines_path: Path,
    mmd_path: Path,
    page_width_pt: float = 595.276,
) -> dict[str, Any]:
    """Conservative residual-fragment cleanup after Lines-first MMD span matching.

    Only rows immediately following an accepted MMD span can be suppressed, and only
    when their normalized content is almost entirely contained, in order, in the MMD
    text already emitted for that span. This is not general deduplication.
    """
    ...
    for i, row in enumerate(rows):
        ...
        for j in range(i + 1, min(len(rows), i + 1 + _MAX_LOOKAHEAD)):
            ...
            if _page(candidate) != page:
                break
            # Never suppress another independently accepted MMD span.
            ...
            if not accepted:
                # Residual fragments are expected to be immediately adjacent. Once a
                # non-duplicate row is reached, do not hop over it looking for deletions.
                break
    ...
    spine["policy"] = (
        "LINES_FIRST_MMD_SPAN_DEDUP preserves Lines-first MMD span output and only suppresses "
        "immediately following same-page Lines fragments that are >=92% contained in order in "
        "the already emitted matched MMD block. General deduplication is forbidden."
    )
...
__all__ = ["build_lines_first_markdown_span_dedup_contract"]
