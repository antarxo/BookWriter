...
function coalesceDocxFlow(input){
  ...
  book.pages=pages.length?pages:book.pages;book.extensions={...(book.extensions||{}),paginationRequired:true,paginationCertified:false};
  return book;
}
...
async function paginateBook(input,options={}){if(!global.BookCore)throw Error('BookCore is required for canonical pagination.');const book=clone(input);await preloadAssets(book,options);const host=makeHost(),report={version:VERSION,startedAt:new Date().toISOString(),sourcePages:(book.pages||[]).length,outputPages:0,splitItems:0,splitByType:{},resizedFigures:0,oversizeItems:[],hardBreaks:0,rendererVersion:BookCore.VERSION};try{const generated=[];for(const section of book.pages||[]){report.hardBreaks+=(section.items||[]).filter(isHardBreak).length;generated.push(...await paginateSection(book,section,options,host,report))}book.pages=generated;book.meta={...(book.meta||{}),authoringVersion:'bookwriter-4.5.0-rc1',updatedAt:new Date().toISOString()};report.outputPages=generated.length;report.completedAt=new Date().toISOString();const audit=await auditOverflow(book,options);report.audit=audit;book.importManifest={...(book.importManifest||{}),pagination:{version:VERSION,generatedAt:new Date().toISOString(),sourcePages:report.sourcePages,outputPages:generated.length,splitItems:report.splitItems,splitByType:report.splitByType,resizedFigures:report.resizedFigures,oversizeItems:report.oversizeItems,hardBreaks:report.hardBreaks,rendererVersion:BookCore.VERSION,audit}};book.extensions={...(book.extensions||{}),paginationRequired:true,paginationCertified:audit.ok,paginationStatus:audit.ok?'certified':'overflow',paginationCertifiedAt:audit.ok?new Date().toISOString():null,paginationCurrentPageCount:generated.length};delete book.extensions.paginationStaleAt;delete book.extensions.paginationStaleReason;delete book.extensions.paginationCertifiedPageCount;delete book.extensions.paginationReconciledAt;delete book.extensions.paginationReconciliation;if(!audit.ok&&options.rejectOverflow!==false)throw Object.assign(new Error(`Η σελιδοποίηση άφησε ${audit.overflowPages} σελίδες με υπερχείλιση.`),{paginationReport:report});return{book,report,audit}}finally{host.remove()}}
