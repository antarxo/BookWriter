...
function sourceRef(ctx,block){return{kind:'docx',sourceFile:ctx.fileName,sourcePage:Number(block.page||ctx.page)||1,sourceParagraph:block.sourceParagraph||null}}
...
function convertBlock(block,ctx){
  ...
  const extBase={docxCanonicalImport:'v4.4',...(hardBreak?{paginationHardBreakBefore:true}:{})};
...
function makeBook(result,entries,id,title,options={}){
  ...
  const items=[],ctx={fileName:result.fileName,page:entries[0]?.page||1,preserveHeadingColors:preserve,imageName,id:block=>block.id?unique(block.id):unique('docx-item-'+(++itemCounter))};
...
function buildInsertionDraft(inputBook,result,entries,anchor,position='after',options={}){
  ...
  const insertionId='docx-insert-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,7),usedIds=bookItemIds(book),existingNames=usedImageNames(book),imageMap=new Map();
  ...
  let counter=0;const ctx={fileName:result.fileName,page:entries[0]?.page||1,preserveHeadingColors:options.preserveHeadingColors!==false,imageName,id:block=>BookModelV4.uniqueId(block.id||`${insertionId}-item-${++counter}`,usedIds)};
  ...
  return{book,pageIndex,insertionId,entries:deepClone(entries),anchor:{pageId:targetPage.id,itemId:anchor.itemId,label:anchor.label||BookModelV4.summarizeItem(targetPage.items[anchorIndex])},position,insertedIds:inserted.map(item=>item.id),firstInsertedId:inserted[0].id,imageMap,duplicateEvidence:evidence};
}
