...
function fillLabsEditor(registry){
  ...
  const insertMode=!!state.labs.insertTarget;
  ...
  $('#labsEditorId')?.closest('label')?.classList.toggle('hidden',insertMode);
  $('#labsEditorStatus')?.closest('label')?.classList.toggle('hidden',insertMode);
  $('#labsSavePreset')?.classList.toggle('hidden',insertMode);
  ...
  const insertButton=$('#labsInsertScene');
  if(insertButton){
...
async function openLabsWorkspace(options={}){
  try{
    await loadLabRegistry();
    if(options.insertPlacement)state.labs.insertTarget={placement:options.insertPlacement==='before'?'before':'after',pageId:page()?.id||'',itemId:item()?.id||'',createdAt:new Date().toISOString()};
    else state.labs.insertTarget=null;
    setView('labs');
    setStatus(options.insertPlacement?'Διάλεξε σκηνή από τα Εργαστήρια και πάτησε «Εισαγωγή στο βιβλίο».':'Άνοιξε ο επεξεργαστής/προεπισκόπηση σκηνών εργαστηρίων.','good');
  }catch(error){
...
async function insertSelectedLabSceneFromWorkspace(){
  ...
  const src=currentLabsEditorUrl()||buildLabSceneSrc(lab,preset,false);
  const title=state.labs.insertTarget?($('#labsEditorTitle')?.value.trim()||preset.title||lab.title||'Ζωντανή σκηνή'):(preset.title||lab.title||'Ζωντανή σκηνή');
  const caption=preset.description||'';
  const width=Number($('#labsEditorWidth')?.value)||preset.layout?.widthPx||560;
  const aspect=String($('#labsEditorAspect')?.value||preset.layout?.aspectRatio||'16/9').trim()||'16/9';
  const layout=Object.assign({placement:'wide',widthPx:560,aspectRatio:'16/9'},preset.layout||{},{
    placement:$('#labsEditorPlacement')?.value||preset.layout?.placement||'wide',
    widthPx:width,
    aspectRatio:aspect
  });
  const appliedQuery=stringValueMap(sceneQueryObject(src));
  const baseQuery=stringValueMap(presetQueryForDialog(lab,preset));
  const queryDiff=diffValueMap(baseQuery,appliedQuery);
  const baseLayout=Object.assign({placement:'wide',widthPx:560,aspectRatio:'16/9'},preset.layout||{});
  const localLayout=diffPlainObject(baseLayout,layout);
  const labSourceRef={
    kind:'lab-registry',
    registry:state.labs.label||LAB_LIBRARY_REGISTRY_PATH,
    labId:lab.id,
    presetId:preset.id,
    appliedQuery,
    localQuery:queryDiff.changed,
    removedQueryKeys:queryDiff.removed,
    appliedLayout:clone(layout),
    localLayout,
    appliedSrc:src
  };
  const labExtension={
    registry:state.labs.label||LAB_LIBRARY_REGISTRY_PATH,
    registrySource:state.labs.source||'',
    labId:lab.id,
    labTitle:lab.title||'',
    presetId:preset.id,
  ...
  mutate(`Σκηνή εργαστηρίου: ${title}`,book=>{
    ...
    const anchor=target.itemId?p.items.findIndex(current=>current.id===target.itemId):-1;
    if(anchor>=0)index=anchor+(target.placement==='before'?0:1);
    else if(target.placement==='before')index=0;
...
function stringValueMap(object={}){
  const out={};
...
function diffValueMap(base={},applied={}){
  const before=stringValueMap(base),after=stringValueMap(applied),changed={},removed=[];
...
function diffPlainObject(base={},applied={}){
  const changed={};
  Object.entries(applied||{}).forEach(([key,value])=>{
    if(JSON.stringify(base?.[key])!==JSON.stringify(value))changed[key]=clone(value);
  });
  return changed;
}
...
function insertItem(forcedType=null,placement='after'){
  return (async()=>{
    ...
    if(!type) return;
    if(type==='scene')return openLabsWorkspace({insertPlacement:placement});
...
function sourceBlockText(block){return sourceIsWeb()?W.blockText(block):D.blockText(block)}
function sourceAudit(result,entries){return sourceIsWeb()?W.audit(result,entries):D.audit(result,entries)}
...
function startDocxInsert(){
  ...
  setView('insert');setStatus('Αριστερά επίλεξε το σημείο στο βιβλίο. Δεξιά άνοιξε DOCX, URL ή HTML και επίλεξε το τμήμα που θα εισαχθεί.');
}
function startWebInsert(){
  startDocxInsert();
  if(state.view==='insert')requestWebUrl();
}
function docxIndex(key){return state.docx.entries.findIndex(entry=>entry.key===key)}
function docxRange(){const all=state.docx.entries||[];if(!all.length)return[];let a=all.findIndex(x=>x.key===state.docx.startKey),b=all.findIndex(x=>x.key===state.docx.endKey);if(a<0)a=0;if(b<0)b=a;if(a>b)[a,b]=[b,a];return all.slice(a,b+1)}
...
function adoptWebResult(result,workflow){
  ...
  resetDocxState(workflow);
  state.docx.file={name:result.fileName||'Ιστοσελίδα'};
  state.docx.result=result;
  state.docx.entries=entries;
  ...
  state.docx.report=W.audit(result,docxRange());
}
async function analyzeWebUrl(url){
  const raw=String(url||'').trim();
  if(!raw)return;
  state.busy=true;renderChrome();setStatus('Ανάγνωση ιστοσελίδας…','warn');
  const workflow={mode:state.docx.mode,insertion:state.docx.insertion};
  try{
    const result=await W.parseUrl(raw);
    adoptWebResult(result,workflow);
    const skipped=result.skippedImages?.length||0;
    setStatus(`Η ιστοσελίδα αναλύθηκε: ${state.docx.entries.length} blocks · ${result.usedImages.length} εικόνες${skipped?' · '+skipped+' εικόνες δεν ήταν αντιγράψιμες':''}.`,skipped?'warn':'good');
  }catch(error){
    console.error(error);
    setStatus('Αποτυχία ιστοσελίδας: '+error.message,'bad');
    modal('Παρεμβολή από ιστοσελίδα',`<div class="info-card bad">${esc(error.message)}</div><div class="info-card warn">Αν η σελίδα δεν επιτρέπει ανάγνωση από τον browser, αποθήκευσέ την ως HTML και χρησιμοποίησε το κουμπί <b>HTML αρχείο…</b>.</div>`);
  }finally{state.busy=false;renderActiveDocxWorkspace()}
}
async function analyzeWebHtmlFile(file){
  if(!file)return;
  state.busy=true;renderChrome();setStatus('Ανάλυση HTML: '+file.name+'…','warn');
  const workflow={mode:state.docx.mode,insertion:state.docx.insertion};
  try{
    const html=await file.text();
    const result=await W.parseHtml(html,{fileName:file.name,baseUrl:location.href});
    adoptWebResult(result,workflow);
    const skipped=result.skippedImages?.length||0;
    setStatus(`Το HTML αναλύθηκε: ${state.docx.entries.length} blocks · ${result.usedImages.length} εικόνες${skipped?' · '+skipped+' εικόνες δεν ήταν αντιγράψιμες':''}.`,skipped?'warn':'good');
  }catch(error){
    console.error(error);
    setStatus('Αποτυχία HTML: '+error.message,'bad');
    modal('Παρεμβολή από HTML',`<div class="info-card bad">${esc(error.message)}</div>`);
  }finally{state.busy=false;renderActiveDocxWorkspace()}
}
function requestWebUrl(){
  const url=prompt('Διεύθυνση ιστοσελίδας για παρεμβολή:', 'https://');
  if(url)analyzeWebUrl(url);
}
...
$('#insertWebFileButton').onclick=()=>$('#insertWebFileInput').click();
$('#insertWebFileInput').onchange=event=>{const file=event.target.files?.[0];event.target.value='';if(file)analyzeWebHtmlFile(file)};
